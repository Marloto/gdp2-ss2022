...
public class Bubblesort {
	public static void sortieren(Vergleichbar[] zuSortieren) { // was wäre das Array?
		...
		while (getauscht) {
			...
			for (int i = 0; i < zuSortieren.length - 1; ++i) {
				// if (zuSortieren[i].getMatrikelnummer() > zuSortieren[i +
				// 1].getMatrikelnummer()) {
	...
	public static void sortieren(Comparable[] zuSortieren) { // was wäre das Array?
		boolean getauscht = true;
		...
		while (getauscht) {
			...
			for (int i = 0; i < zuSortieren.length - 1; ++i) {
				// if (zuSortieren[i].getMatrikelnummer() > zuSortieren[i +
				// 1].getMatrikelnummer()) {
				// was muss hier anders werden?
				if (zuSortieren[i].compareTo(zuSortieren[i + 1]) > 0) {
					Comparable tmp = zuSortieren[i];
					zuSortieren[i] = zuSortieren[i + 1];
					zuSortieren[i + 1] = tmp;
					getauscht = true;
				}
